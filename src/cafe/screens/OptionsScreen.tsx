...
const OptionsScreen: React.FC<OptionsScreenProps> = ({
  ...
  const isDessert = menuItem.category === 'dessert';
  const isCoffee = menuItem.category === 'coffee';
  const canSelectIced = menuItem.availableIced;

  const handleAddToCart = () => {
    const options: OrderOption = isDessert
      ? { size: 'Medium', temperature: 'Hot', extraShots: 0, sweetness: 'Normal' }
      : { size, temperature, extraShots, sweetness, decaf: isCoffee ? decaf : undefined };
          ...
          {isDessert ? (
            ...
            <div className="option-group">
              ...
              <div className="counter-buttons">
                <button className="counter-btn" onClick={() => setQuantity(Math.max(1, quantity - 1))}>−</button>
                <span className="counter-value">{quantity}개</span>
                <button className="counter-btn" onClick={() => setQuantity(Math.min(10, quantity + 1))}>+</button>
              </div>
            </div>
          ) : (
            <>
              ...
              {isCoffee && (
                <div className="option-group">
                  <label className="option-label">디카페인 (Decaf)</label>
                  <div className="option-buttons">
                    <button
                      className={`option-btn ${!decaf ? 'active' : ''}`}
              ...
              <div className="option-group">
                ...
                <div className="counter-buttons">
                  <button className="counter-btn" onClick={() => setExtraShots(Math.max(0, extraShots - 1))}>−</button>
                  <span className="counter-value">{extraShots}샷</span>
                  <button className="counter-btn" onClick={() => setExtraShots(Math.min(5, extraShots + 1))}>+</button>
                </div>
              </div>
              ...
              <div className="option-group">
                ...
                <div className="option-buttons">
                  <button className={`option-btn ${sweetness === 'Less' ? 'active' : ''}`} onClick={() => setSweetness('Less')}>덜 달게</button>
                  <button className={`option-btn ${sweetness === 'Normal' ? 'active' : ''}`} onClick={() => setSweetness('Normal')}>보통</button>
                  <button className={`option-btn ${sweetness === 'Extra' ? 'active' : ''}`} onClick={() => setSweetness('Extra')}>더 달게</button>
                </div>
              </div>
