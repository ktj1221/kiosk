...
const menuData = [
  { id: 1, name: '아메리카노', category: 'recommend', price: 2500, emoji: '☕', image: OFFICIAL_IMG.coffeeChocoGelato, canSize: true, canCoffeeType: true },
  { id: 2, name: '카페라떼', category: 'recommend', price: 3400, emoji: '🥛', image: OFFICIAL_IMG.appleShineMuscat, canSize: true, canCoffeeType: true },
  { id: 3, name: '카푸치노', category: 'recommend', price: 3700, emoji: '☕', image: OFFICIAL_IMG.mangoJasmine, canSize: true, canCoffeeType: true },
  { id: 4, name: '바닐라라떼', category: 'recommend', price: 3900, emoji: '🍦', image: OFFICIAL_IMG.mgcJuice, canSize: true, canCoffeeType: true },
  { id: 5, name: '콜드브루', category: 'recommend', price: 3700, emoji: '🧊', image: OFFICIAL_IMG.pistachioFrappe, canSize: true, canCoffeeType: true },
  { id: 6, name: '유자차', category: 'recommend', price: 3200, emoji: '🍊', image: OFFICIAL_IMG.strawberryLatte, canSize: false, canCoffeeType: false },

  { id: 11, name: '아메리카노', category: 'coffee', price: 2500, emoji: '☕', image: OFFICIAL_IMG.coffeeChocoGelato, canSize: true, canCoffeeType: true },
  { id: 12, name: '카페라떼', category: 'coffee', price: 3400, emoji: '☕', image: OFFICIAL_IMG.appleShineMuscat, canSize: true, canCoffeeType: true },
  { id: 13, name: '카푸치노', category: 'coffee', price: 3700, emoji: '☕', image: OFFICIAL_IMG.mangoJasmine, canSize: true, canCoffeeType: true },
  { id: 14, name: '바닐라라떼', category: 'coffee', price: 3900, emoji: '☕', image: OFFICIAL_IMG.mgcJuice, canSize: true, canCoffeeType: true },
  { id: 15, name: '에스프레소', category: 'coffee', price: 2800, emoji: '☕', image: OFFICIAL_IMG.milkShake, canSize: false, canCoffeeType: true },
  { id: 16, name: '콜드브루', category: 'coffee', price: 3700, emoji: '🧊', image: OFFICIAL_IMG.pistachioFrappe, canSize: true, canCoffeeType: true },

  { id: 41, name: '그린티 라떼', category: 'smoothie', price: 4200, emoji: '🍵', image: OFFICIAL_IMG.blueberryYogurtSmoothie, canSize: true, canCoffeeType: false },
  { id: 42, name: '아이스티', category: 'smoothie', price: 3000, emoji: '🧋', image: OFFICIAL_IMG.blueberryPlumJuice, canSize: true, canCoffeeType: false },

  { id: 51, name: '유자 에이드', category: 'ade', price: 3800, emoji: '🍋', image: OFFICIAL_IMG.strawberryLatte, canSize: true, canCoffeeType: false },
  { id: 52, name: '블랙티 에이드', category: 'ade', price: 3800, emoji: '🫖', image: OFFICIAL_IMG.goldKiwiJuice, canSize: true, canCoffeeType: false },

  { id: 61, name: '유자차', category: 'tea', price: 3200, emoji: '🍊', image: OFFICIAL_IMG.strawberryLatte, canSize: false, canCoffeeType: false },
  { id: 62, name: '블랙티', category: 'tea', price: 3000, emoji: '🫖', image: OFFICIAL_IMG.goldKiwiJuice, canSize: false, canCoffeeType: false },
  { id: 63, name: '그린티', category: 'tea', price: 3200, emoji: '🍵', image: OFFICIAL_IMG.blueberryYogurtSmoothie, canSize: false, canCoffeeType: false },

  { id: 81, name: '엠지씨네 양념 컵치킨', category: 'dessert', price: 3900, emoji: '🍗', image: OFFICIAL_IMG.seasonedCupChicken, canSize: false, canCoffeeType: false },
  { id: 82, name: '에그머니 머핀인줄 브런치빵', category: 'dessert', price: 3600, emoji: '🧁', image: OFFICIAL_IMG.eggMuffinBrunchBread, canSize: false, canCoffeeType: false },
  { id: 83, name: '컵치킨+브런치빵 세트', category: 'dessert', price: 6900, emoji: '🍱', image: OFFICIAL_IMG.seasonedCupChicken, canSize: false, canCoffeeType: false },
  { id: 84, name: '브런치빵', category: 'dessert', price: 3500, emoji: '🥯', image: OFFICIAL_IMG.eggMuffinBrunchBread, canSize: false, canCoffeeType: false },

  { id: 71, name: '오늘의 할인 아메리카노', category: 'discount', price: 2000, emoji: '🔥', image: OFFICIAL_IMG.coffeeChocoGelato, canSize: true, canCoffeeType: true },
  { id: 72, name: '오늘의 할인 카페라떼', category: 'discount', price: 2900, emoji: '🔥', image: OFFICIAL_IMG.appleShineMuscat, canSize: true, canCoffeeType: true }
];
...
function updateViewportMode() {
  const kioskWidth = Math.min(window.innerWidth, (window.innerHeight * 9) / 16);
  document.body.classList.toggle('is-compact', kioskWidth < 900);
  document.body.classList.toggle('is-narrow', kioskWidth < 620);
}
...
function openModal(id) {
  ...
  const typeGroup = document.getElementById('opt-group-type');
  if (item.canCoffeeType) {
    typeGroup.style.display = '';
    document.querySelectorAll('[data-group="type"]').forEach((chip) => {
      chip.classList.toggle('active', chip.dataset.val === 'ICE');
    });
  } else {
...
function confirmAddToCart() {
  ...
  const sizeLabel = modalItem.canSize ? (modalSize === 'L' ? 'L' : 'M') : '';
  const typeLabel = modalItem.canCoffeeType ? modalType : '';
  const optionLabel = [typeLabel, sizeLabel].filter(Boolean).join(' / ');
  const cartKey = `${modalItem.id}_${typeLabel || 'none'}_${sizeLabel || 'none'}`;
  ...
  list.innerHTML = cart.map((item) => `
    <div class="preview-item">
      ...
      <div class="preview-controls">
        <button class="preview-btn" onclick="changeQty('${item.cartKey}',-1)">-</button>
        <span class="preview-qty">${item.qty}</span>
        <button class="preview-btn" onclick="changeQty('${item.cartKey}',1)">+</button>
        <button class="preview-del-btn" onclick="removeItem('${item.cartKey}')">삭제</button>
      </div>
    </div>
  `).join('');
}
...
function scrollPreview(direction) {
  const list = document.getElementById('order-preview-list');
  if (!list) return;
  const step = Math.max(80, Math.floor(list.clientHeight * 0.45));
